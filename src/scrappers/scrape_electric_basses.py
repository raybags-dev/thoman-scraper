...
async def process_endpoints_and_save_base_guitar_data(run_scraper: bool = None, depth: int = None):
    electric_bass_url = "https://www.thomann.de/gb/all-products-from-the-category-electric-basses.html"
    bass_guitar_endpoints_file_path = "src/endpoints/bass_guitar_endpoints/bass_guitar_endpoints.csv"
    parsed_data_bass_file_path = "src/data/parsed_data_b_guitars.csv"
    ...
    if not Path(bass_guitar_endpoints_file_path).is_file() or Path(bass_guitar_endpoints_file_path).stat().st_size == 0:
        my_log(message="Endpoints file does not exist or is empty. Generating new endpoints.", log_type="info")
        run_scraper = await main_paginator(electric_bass_url, bass_guitar_endpoints_file_path)
        if not run_scraper:
    ...
    # Clear files only if the scraper is running
    # if run_scraper:
    #     clear_files(bass_guitar_endpoints_file_path, parsed_data_bass_file_path)

    # Load endpoints from file
    try:
        endpoints_df = pd.read_csv(bass_guitar_endpoints_file_path)
    ...
    async def process_endpoint(endpoint):
        try:
            ...
            if content:
                await parse_and_save_data(page_content=content, data_file=parsed_data_bass_file_path)
